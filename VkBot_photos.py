...
vk_session = vk_api.VkApi(token=credentials.get('group').get('token'))
upload = vk_api.VkUpload(vk_session)
session_api = vk_session.get_api()
longpoll = VkBotLongPoll(vk_session, group_id=credentials.get('group').get('id_group'))

# Переключение на пользователя
def UserSessionAuth():
    login = get_config('credentials').get('user').get('login')
    password = get_config('credentials').get('user').get('password')
...
def getUserPhoto(event):
    id_group = credentials.get('group').get('id_group')
    img_data = 0
    code = False
    for i in range(len(config.get('photo_sizes'))):
        for j in range(len(event.object['message']['attachments'][0]['photo']['sizes'])):
            if event.object['message']['attachments'][0]['photo']['sizes'][j]['type'] == config.get('photo_sizes')[i]:
    ...
    if code == False:
        vk_session.method('messages.send', {'user_id': event.object['message']['from_id'],
                                            'message': config.get('users').get('photo').get('size_error'),
                                            'random_id': 0})
        return
    id = uuid.uuid4()
    img_name = f'{id}.jpg'
    with open(f'{config.get("media_folder")}/{id_group}/user/' + img_name, 'wb') as userphoto:
        userphoto.write(img_data)
    print(event.object)

    # Отправление ответного сообщения пользователю (та же фотография)
    photo = upload.photo_messages(f'{config.get("media_folder")}/{id_group}/user/' + img_name)[0]
    attachments = list()
...
def getWallPhoto(event):
    ...
    for i in range(len(event.object['attachments'])):
        if event.object['attachments'][i]['type'] == 'photo':
            ...
            id = uuid.uuid4()
            name_img = f'{id}.jpg'
            with open(f'{config.get("media_folder")}/{id_group}/' + name_img, 'wb') as photo:
        ...
        for i in range(len(photos['items'])):
            ...
            for j in range(len(config.get('photo_sizes'))):
                for k in range(len(photos['items'][i]['sizes'])):
                    if photos['items'][i]['sizes'][k]['type'] == config.get('photo_sizes')[j]:
            ...
            id = uuid.uuid4()
            img_name = f'{id}.jpg'
            with open(f'{config.get("media_folder")}/{id_group}/' + img_name, 'wb') as photo:
                photo.write(img_data)
    ...
    if event == None:
        ...
        for i in range(len(config.get('photo_sizes'))):
            for j in range(len(event.object['sizes'])):
                if event.object['sizes'][j]['type'] == config.get('photo_sizes')[i]:
